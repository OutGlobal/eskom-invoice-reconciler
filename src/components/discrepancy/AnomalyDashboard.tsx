...
export const AnomalyDashboard: React.FC<AnomalyDashboardProps> = ({ initialSummary }) => {
        ...
        comparisons: [
          {
            ...
            tolerance: {
              ...
              absolute_tolerance_zar: new Decimal("100"),
          ...
          },
          {
            ...
            tolerance: {
              component_code: "REACTIVE_PENALTY_CHARGES",
              component_name: "Reactive Penalty",
              absolute_tolerance_zar: new Decimal("10"),
              percentage_tolerance: new Decimal("0.001"),
              unit: "ZAR",
            },
        ...
        discrepancies: [
          {
            ...
            root_cause_description:
              "Billed demand (4,850 kVA) exceeds ratcheted contracted NMD floor ceiling.",
          },
        ],
  ...
  const handleCreateDisputeItem = (d: DiscrepancyDiagnosis) => {
    toast.success(
      `Dispute line item created for ${d.reason_code} (Impact: R ${d.estimated_financial_impact_zar.toFixed(2)})`,
    );
  };
    ...
    <div className="space-y-6">
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground font-mono">
              Run ID: {activeSummary.reconciliation_run_id || "LOCAL-SCAN"}
            </span>
          </div>
          <h2 className="text-2xl font-bold tracking-tight text-foreground">
            Discrepancy Analysis & Anomaly Dashboard
          </h2>
          <p className="text-sm text-muted-foreground">
            Rule-based variance root cause identification across 22 billing, TOU schedule,
            telemetry, and rate categories.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-card border border-border rounded-xl p-5 shadow-sm space-y-2">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-bold text-foreground">
              {activeSummary.total_diagnoses}
            </span>
            <span className="text-xs text-muted-foreground">anomalies diagnosed</span>
          </div>
          <div className="text-[11px] text-muted-foreground">
            {activeSummary.critical_count} Critical · {activeSummary.high_count} High ·{" "}
            {activeSummary.medium_count} Medium
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-5 shadow-sm space-y-2">
          ...
          <div className="flex items-baseline gap-1 text-red-600 dark:text-red-400">
            ...
            <span className="text-3xl font-bold">
              {activeSummary.total_disputed_financial_impact_zar.toFixed(2)}
            </span>
          </div>
          <div className="text-[11px] text-muted-foreground">
            Total calculated overcharge & rate variance
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-5 shadow-sm space-y-2">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-bold text-foreground">
              {activeSummary.total_diagnoses > 0
                ? (
                    (activeSummary.high_confidence_count / activeSummary.total_diagnoses) *
                    100
                  ).toFixed(0)
                : 100}
              %
            </span>
            <span className="text-xs text-emerald-500 font-medium font-mono">
              High Empirical Proof
            </span>
          </div>
          <div className="text-[11px] text-muted-foreground">
            {activeSummary.high_confidence_count} diagnoses with 100% deterministic evidence
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-5 shadow-sm space-y-2">
          ...
          <div className="text-[11px] text-muted-foreground">
            0% LLM hallucination risk · Pure domain rules
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-semibold text-foreground flex items-center gap-2">
            <FileText className="h-5 w-5 text-primary" /> Diagnosed Variance Root Causes (
            {filteredDiagnoses.length})
          </h3>
          <span className="text-xs text-muted-foreground">
            Showing deterministic diagnostic results
          </span>
        </div>
        ...
        {filteredDiagnoses.length === 0 ? (
          <div className="bg-card border border-border rounded-xl p-12 text-center space-y-3">
            ...
            <p className="text-sm text-muted-foreground max-w-md mx-auto">
              No variance diagnoses match your selected filters. All evaluated components passed
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                  <div>
                    ...
                    <div className="font-mono font-semibold text-foreground mt-0.5">
                      {d.affected_billing_component}
                    </div>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Affected Records Count:</span>
                    <div className="font-semibold text-foreground mt-0.5">
                      {d.affected_records_count} records / line items
                    </div>
                  </div>
                  <div>
                    <span className="text-muted-foreground">NERSA / Regulatory Ref:</span>
                    <div
                      className="font-medium text-primary truncate mt-0.5"
                      title={d.nersa_reference || "N/A"}
                    >
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2">
                  <div className="text-xs text-muted-foreground flex items-center gap-1.5">
                    ...
                    <span>
                      Action:{" "}
                      <strong>{d.recommended_action || "Review and submit dispute pack."}</strong>
                    </span>
                  </div>
