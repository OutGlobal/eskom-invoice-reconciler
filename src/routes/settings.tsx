  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-muted-foreground">Application preferences and data management.</p>
      </div>

      <Panel title="Application Info">
        <dl className="text-sm grid grid-cols-1 sm:grid-cols-2 gap-y-2">
          <dt className="text-muted-foreground">Loaded intervals</dt><dd>{rows.length.toLocaleString()}</dd>
          <dt className="text-muted-foreground">Uploads this session</dt><dd>{uploads.length}</dd>
          <dt className="text-muted-foreground">Interval resolution</dt><dd>30 minutes</dd>
          <dt className="text-muted-foreground">Power factor</dt><dd>0.96</dd>
        </dl>
      </Panel>

      <Panel title="Future Readiness"
        ...
        <ul className="text-sm list-disc pl-5 space-y-1 text-muted-foreground">
          <li>Multiple customers &amp; meters (extend <code>store.ts</code> with arrays)</li>
      ...
      <Panel title="Data Management">
        <button onClick={reset} className="rounded-md border border-red-500/40 bg-red-500/10 text-red-500 px-4 py-2 text-sm hover:bg-red-500/20">
