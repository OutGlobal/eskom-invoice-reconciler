...
import {
  ...
  type TariffValidationError,
} from "@/domain/tariff/tariffValidationEngine";
...
function TariffPage() {
  ...
  useEffect(() => {
    async function loadTariffs() {
      ...
      const matching =
        allVersions.find((v) => v.header.tariff_family === selectedFamily) || allVersions[0];
      if (matching) {
  ...
  const handleVersionChange = (versionId: string) => {
    ...
    const matched = versions.find(
      (v) => `${v.header.tariff_code}_${v.header.version}` === versionId,
    );
    if (matched) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border pb-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Deterministic gazetted rate schedules for Eskom &amp; Municipalities. Tariffs are
            strictly stored as versioned data.
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="rounded-lg border border-border bg-card p-3 space-y-2">
              ...
              >
                {versions
                  ...
                  .map((v) => (
                    <option
                      key={`${v.header.tariff_code}_${v.header.version}`}
                      value={`${v.header.tariff_code}_${v.header.version}`}
                    >
            ...
            <div className="rounded-lg border border-border bg-card p-3 space-y-1">
              <div className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                <ShieldCheck className="h-3.5 w-3.5 text-emerald-500" /> Gazette Lineage
                Verification
              </div>
                      ...
                      {highSeasonComponents.map((comp) => (
                        <tr key={comp.rule_id} className="hover:bg-muted/20">
                          ...
                          <td className="p-2 font-mono text-right">
                            {comp.rate_value.toFixed(4)} {comp.unit_of_measure}
                          </td>
              ...
              <div className="space-y-2 text-xs">
                <div className="flex justify-between py-1 border-b border-border">
                  ...
                  <span className="font-mono font-medium">
                    R {activeVersion.reactive_penalty_rate.toFixed(4)} / kVARh
                  </span>
                </div>
                <div className="flex justify-between py-1 border-b border-border">
                  <span className="text-muted-foreground">Power Factor Threshold:</span>
                  <span className="font-mono font-medium">
                    {activeVersion.pf_threshold.toFixed(2)} (lagging)
                  </span>
                </div>
                <div className="flex justify-between py-1">
                  <span className="text-muted-foreground">Minimum NMD Threshold:</span>
                  <span className="font-mono font-medium">
                    {activeVersion.minimum_nmd_kva.toFixed(0)} kVA
                  </span>
                </div>
              </div>
            </Panel>
              ...
              <div className="space-y-2 text-xs">
                <div className="flex justify-between py-1 border-b border-border">
                  ...
                  <span className="font-mono font-medium text-amber-500">
                    {activeVersion.nmd_ratchet_multiplier.toFixed(1)}x Gazetted Rate
                  </span>
                </div>
                <div className="flex justify-between py-1 border-b border-border">
                ...
                <div className="flex justify-between py-1">
                  ...
                  <span className="font-mono text-[10px] text-muted-foreground">
                    max(0, Demand - NMD) &times; Rate &times; 2.0
                  </span>
                </div>
              </div>
            </Panel>
          ...
          <div className="w-full max-w-xl rounded-lg border border-border bg-card p-6 shadow-lg space-y-4">
            ...
            <div className="space-y-3 text-xs">
              ...
              <div className="grid grid-cols-2 gap-2 text-muted-foreground">
                <div>
                  <span className="font-semibold text-foreground">Tariff Name:</span>{" "}
                  {explainerData.tariff_name}
                </div>
                <div>
                  <span className="font-semibold text-foreground">Tariff Code:</span>{" "}
                  {explainerData.tariff_code}
                </div>
                <div>
                  <span className="font-semibold text-foreground">Version:</span>{" "}
                  {explainerData.version_number}
                </div>
                <div>
                  <span className="font-semibold text-foreground">Effective Date:</span>{" "}
                  {explainerData.effective_date}
                </div>
                <div>
                  <span className="font-semibold text-foreground">Component Code:</span>{" "}
                  {explainerData.component_code}
                </div>
                <div>
                  <span className="font-semibold text-foreground">Rate Value:</span>{" "}
                  {explainerData.rate_value} {explainerData.unit_of_measure}
                </div>
                <div>
                  <span className="font-semibold text-foreground">Season:</span>{" "}
                  {explainerData.season.toUpperCase()}
                </div>
                <div>
                  <span className="font-semibold text-foreground">TOU Period:</span>{" "}
                  {explainerData.tou_period.toUpperCase()}
                </div>
              </div>
