    ...
    <div className="space-y-6">
      ...
      <Panel title="Fixed Charges (R/kVA/month)" subtitle="Applied to Notified Maximum Demand.">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          <NumField
            ...
            value={t.networkCapacity}
            onChange={(v) => upd({ networkCapacity: v })}
          />
          <NumField
            label="Network Demand"
            value={t.networkDemand}
            onChange={(v) => upd({ networkDemand: v })}
          />
          <NumField
            label="Generation Capacity"
            value={t.generationCapacity}
            onChange={(v) => upd({ generationCapacity: v })}
          />
          <NumField
            label="Transmission Network"
            value={t.transmissionNetwork}
            onChange={(v) => upd({ transmissionNetwork: v })}
          />
        </div>
      </Panel>
      ...
      <Panel title="Additional Charges (c/kWh)" subtitle="Applied to total kWh.">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <NumField
            ...
            value={t.ancillary}
            onChange={(v) => upd({ ancillary: v })}
          />
          <NumField
            label="Electrification &amp; Rural"
            value={t.electrification}
            onChange={(v) => upd({ electrification: v })}
          />
          <NumField
            label="Affordability Subsidy"
            value={t.affordability}
            onChange={(v) => upd({ affordability: v })}
          />
        </div>
      </Panel>

      <Panel
        title="Active Energy Rates (c/kWh)"
        subtitle="High-demand season = Jun-Aug; Low-demand season = Sep-May."
      >
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <NumField
                label="Peak"
                value={t.energy.high.peak}
                onChange={(v) => updEnergy("high", "peak", v)}
              />
              <NumField
                label="Standard"
                value={t.energy.high.standard}
                onChange={(v) => updEnergy("high", "standard", v)}
              />
              <NumField
                label="Off-Peak"
                value={t.energy.high.offPeak}
                onChange={(v) => updEnergy("high", "offPeak", v)}
              />
            </div>
          </div>
          <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <NumField
                label="Peak"
                value={t.energy.low.peak}
                onChange={(v) => updEnergy("low", "peak", v)}
              />
              <NumField
                label="Standard"
                value={t.energy.low.standard}
                onChange={(v) => updEnergy("low", "standard", v)}
              />
              <NumField
                label="Off-Peak"
                value={t.energy.low.offPeak}
                onChange={(v) => updEnergy("low", "offPeak", v)}
              />
            </div>
          </div>
        </div>
      ...
      <Panel title="Time-of-Use Definitions" subtitle="Eskom Megaflex Appendix A.">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
          <TouCard title="Peak" color="#ef4444">
            High: Mon-Fri 06:00-09:00, 17:00-19:00.
            <br />
            Low: Mon-Fri 07:00-10:00, 18:00-20:00.
          </TouCard>
          <TouCard title="Standard" color="#f59e0b">
            High: Mon-Fri 09:00-17:00, 19:00-22:00; Sat 07:00-12:00, 18:00-20:00.
            <br />
      ...
      <Panel title="NMD Rules" subtitle="Notified Maximum Demand controls capacity-based charges.">
        <ul className="text-sm list-disc pl-5 space-y-1 text-muted-foreground">
          ...
          <li>
            Applied to Transmission Network, Distribution Network Capacity, and Generation Capacity
            charges.
          </li>
          <li>
            Exceeding NMD triggers utilised-capacity penalties (Eskom Schedule of Standard Prices).
          </li>
        </ul>
      </Panel>
    </div>
