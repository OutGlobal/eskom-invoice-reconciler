...
import {
  ...
  type LoadTestResult,
} from "@/domain/telemetry/loadTestBenchmarkEngine";
import type {
...
export const Route = createFileRoute("/telemetry")({
  head: () => ({
    meta: [{ title: "AMR Telemetry & Time-Series Validation Engine — Eskom Reconciler" }],
  }),
...
function TelemetryPage() {
  ...
  const [workspaceTab, setWorkspaceTab] = useState<"stream" | "energy" | "demand" | "meters">(
    "stream",
  );
  const [activeTab, setActiveTab] = useState<"stream" | "gaps" | "quarantine" | "benchmark">(
    "stream",
  );
  ...
  const [estimationMethod, setEstimationMethod] =
    useState<EstimationMethod>("LINEAR_INTERPOLATION");
  ...
  useEffect(() => {
    if (storeRows && storeRows.length > 0) {
      ...
      try {
        const rawInputs: RawTelemetryRowInput[] = storeRows.map((r, idx) => ({
          meter_id: customer.meter || "7856504226",
          timestamp: r.ts instanceof Date ? r.ts.toISOString() : String(r.ts),
          channel: "kWh",
          raw_value: (r as any).kWh || (r as any).kwh || (r as any).active_kwh || 0,
          unit: "kWh",
          ingestion_batch_id: "batch-store-init",
          row_number: idx + 1,
          raw_snippet: JSON.stringify(r),
        }));
  ...
  const runInitialDemoStream = () => {
    handleRunBenchmark(500);
  };

  const handleRunBenchmark = (count: number) => {
    setIsProcessing(true);
    toast.loading(`Running load-test benchmark with ${count.toLocaleString()} intervals...`, {
      id: "bench",
    });
  ...
  const handleConfirmEstimation = async () => {
    ...
    try {
      ...
      setEstimationReason("");
      toast.success(
        `Interval at ${selectedGap.expected_interval} estimated cleanly using ${estimationMethod}!`,
      );
    } catch (err: any) {
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 p-6 rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-sm">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Consolidated Ingestion, TOU Load Profiling, NMD Exceedance Diagnostics & Meter
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="p-4 rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-xs">
                ...
                <span className="text-2xs text-gray-400 mt-1 block">
                  Never automatically replaced with zero
                </span>
              </div>
              ...
              <div className="flex items-center gap-2 flex-wrap text-xs">
                {(
                  ...
                  <div
                    key={st}
                    onClick={() =>
                      setSelectedQualityState(selectedQualityState === st ? "ALL" : st)
                    }
            ...
            <div className="bg-white dark:bg-gray-900 rounded-xl shadow-lg border border-gray-200 dark:border-gray-800 overflow-hidden">
                      ...
                      <tbody>
                        {paginatedIntervals.length === 0 ? (
                          ...
                          paginatedIntervals.map((item) => (
                            <tr
                              key={item.id}
                              className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50/50"
                            >
                              <td className="py-2.5 px-4 font-mono text-gray-600 dark:text-gray-400">
                                {item.timestamp_utc}
                              </td>
                              ...
                              <td className="py-2.5 px-4 font-mono text-gray-700 dark:text-gray-300">
                                {item.raw_value}
                              </td>
                              <td className="py-2.5 px-4 font-mono text-gray-700 dark:text-gray-300">
                ...
                <div className="p-6 space-y-4">
                  <div className="flex items-center justify-between border-b pb-3 dark:border-gray-800">
                    <h3 className="font-bold text-gray-900 dark:text-gray-100 text-sm flex items-center gap-2">
                      <AlertTriangle className="w-5 h-5 text-purple-600" /> Missing Interval Gap
                      Analytics & Estimation Framework
                    </h3>
                    <span className="text-xs text-gray-500">
                      Rule: Missing telemetry is NEVER automatically replaced with zero.
                    </span>
                  </div>
                      ...
                      <tbody>
                        {gaps.length === 0 ? (
                          <tr>
                            <td
                              colSpan={7}
                              className="py-8 text-center text-emerald-600 bg-emerald-50 dark:bg-emerald-950/30 rounded-lg"
                            >
                              <CheckCircle2 className="w-6 h-6 mx-auto mb-1" />
                              <p className="font-bold">
                                No missing telemetry interval gaps detected in current stream!
                              </p>
                            </td>
                          </tr>
                        ) : (
                          gaps.map((gap) => (
                            ...
                            >
                              ...
                              <td className="py-3 px-4 text-gray-600">
                                {gap.received_interval || "N/A"}
                              </td>
                              <td className="py-3 px-4 font-semibold text-gray-900 dark:text-gray-100">
                                {gap.missing_duration_minutes} mins
                              </td>
                              <td className="py-3 px-4">
                                <span
                                  className={`px-2 py-0.5 rounded text-2xs font-bold ${gap.quality_impact === "HIGH" ? "bg-red-100 text-red-800" : "bg-amber-100 text-amber-800"}`}
                                >
                                  {gap.quality_impact}
                                </span>
                              </td>
              ...
              {activeTab === "quarantine" && (
                <div className="p-6 space-y-4">
                  <div className="flex items-center justify-between border-b pb-3 dark:border-gray-800">
                    <h3 className="font-bold text-gray-900 dark:text-gray-100 text-sm flex items-center gap-2">
                      <AlertCircle className="w-5 h-5 text-red-600" /> Non-Destructive Quarantine
                      Ledger ({quarantine.length})
                    </h3>
                    <span className="text-xs text-gray-500">
                      Bad data is stored immutably in quarantine for audit review.
                    </span>
                  </div>
                        ...
                        {quarantine.length === 0 ? (
                          <tr>
                            ...
                            >
                              <CheckCircle2 className="w-6 h-6 mx-auto mb-1" />
                              <p className="font-bold">
                                Zero records quarantined. 100% clean telemetry stream.
                              </p>
                            </td>
                          </tr>
                        ) : (
                          quarantine.map((q) => (
                            ...
                            >
                              ...
                              <td className="py-3 px-4 font-bold text-red-600 dark:text-red-400 font-mono">
                                {q.validation_code}
                              </td>
                  ...
                  <div className="flex items-center justify-between border-b pb-3 dark:border-gray-800">
                    <div>
                      <h3 className="font-bold text-gray-900 dark:text-gray-100 text-base flex items-center gap-2">
                        <Zap className="w-5 h-5 text-indigo-600" /> High-Volume Load-Test Benchmark
                        Engine
                      </h3>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                        Verify 10,000, 100,000, and 1,000,000 interval stream ingestion & validation
                        without browser memory lockup.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="p-5 rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-950 space-y-3">
                      ...
                      <p className="text-xs text-gray-600 dark:text-gray-400">
                        Simulates ~208 days of 30-minute interval readings for a single industrial
                        site.
                      </p>
                      <button
                        onClick={() => handleRunBenchmark(10000)}
                    ...
                    <div className="p-5 rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-950 space-y-3">
                      ...
                      <p className="text-xs text-gray-600 dark:text-gray-400">
                        Simulates ~5.7 years of 30-minute interval readings or multi-meter facility
                        fleet data.
                      </p>
                      <button
                        onClick={() => handleRunBenchmark(100000)}
                    ...
                    <div className="p-5 rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-950 space-y-3">
                      ...
                      <p className="text-xs text-gray-600 dark:text-gray-400">
                        Multi-year enterprise municipality dataset. Validates high-volume chunked
                        throughput.
                      </p>
                      <button
                  ...
                  {benchmarkResult && (
                    <div className="p-6 rounded-xl bg-indigo-50/60 dark:bg-indigo-950/40 border border-indigo-200 dark:border-indigo-900 space-y-4">
                      <h4 className="font-bold text-indigo-900 dark:text-indigo-200 text-sm flex items-center gap-2">
                        <CheckCircle2 className="w-5 h-5 text-indigo-600" /> Benchmark Execution
                        Results ({benchmarkResult.targetIntervalCount.toLocaleString()} Intervals)
                      </h4>

                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
                        <div className="p-3 bg-white dark:bg-gray-900 rounded-lg border border-indigo-100">
                          <span className="text-2xs text-gray-500 font-semibold block">
                            Execution Duration
                          </span>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl max-w-lg w-full p-6 shadow-2xl border border-gray-200 dark:border-gray-800 space-y-4">
            <div className="flex items-center justify-between border-b pb-3 dark:border-gray-800">
              <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100 flex items-center gap-2">
                <Sparkles className="w-5 h-5 text-purple-600" /> Explicit Telemetry Interval
                Estimation
              </h3>
              <button
            ...
            <div className="space-y-3 text-xs">
              <div className="p-3 rounded-lg bg-purple-50 dark:bg-purple-950/40 border border-purple-200 dark:border-purple-900">
                <p className="text-purple-900 dark:text-purple-200 font-semibold">
                  Missing Target Interval:{" "}
                  <code className="font-mono font-bold">{selectedGap.expected_interval}</code>
                </p>
                <p className="text-purple-700 dark:text-purple-300 text-2xs mt-0.5">
                  Missing Duration: {selectedGap.missing_duration_minutes} mins | Quality Impact:{" "}
                  {selectedGap.quality_impact}
                </p>
              </div>
              ...
              <div>
                ...
                >
                  <option value="LINEAR_INTERPOLATION">
                    Linear Interpolation (T_{"i-1"} & T_{"i+1"} weighting)
                  </option>
                  <option value="SAME_DAY_PRIOR_WEEK">
                    Same-Day Prior Week Reference (7 days prior)
                  </option>
