...
export async function runTelemetrySubsystemTests() {
  ...
  assert(
    res1.validIntervals[0].quality_state === "ACTUAL",
    "Clean reading classified as ACTUAL quality state",
  );
  assert(res1.validIntervals[0].channel === "kWh", "Channel normalized to kWh");
  assert(
    res1.validIntervals[0].local_timestamp.includes("10:00:00"),
    "Local SAST time normalized (08:00 UTC -> 10:00 SAST)",
  );
  ...
  assert(
    res2.quarantineRecords[0].validation_code === "INVALID_TIMESTAMP_SCHEMA",
    "Quarantine code set to INVALID_TIMESTAMP_SCHEMA",
  );
  ...
  assert(
    res3.quarantineRecords.some((q) => q.validation_code === "NEGATIVE_VALUE_REJECTED"),
    "Detected NEGATIVE_VALUE_REJECTED",
  );
  assert(
    res3.quarantineRecords.some((q) => q.validation_code === "IMPOSSIBLE_DEMAND_CAPACITY"),
    "Detected IMPOSSIBLE_DEMAND_CAPACITY",
  );
  ...
  assert(
    res4.missingGaps.length === 3,
    `Detected exactly 3 missing interval gaps (actual: ${res4.missingGaps.length})`,
  );
  assert(
    res4.missingGaps[0].expected_interval === "2026-03-01T08:30:00.000Z",
    "Identified 08:30 missing interval",
  );
  ...
  assert(
    estimationRes.estimationRecord.method === "LINEAR_INTERPOLATION",
    "Method set to LINEAR_INTERPOLATION",
  );
  assert(estimationRes.estimationRecord.confidence_score === 0.95, "Confidence score set to 0.95");
  assert(
    estimationRes.estimationRecord.source_intervals.length === 2,
    "Logged 2 source interval references",
  );
  assert(
    estimationRes.estimatedInterval.quality_state === "ESTIMATED",
    "Estimated interval classified as ESTIMATED",
  );
  assert(
    estimationRes.estimatedInterval.engineering_value > 0,
    "Estimated interval value is non-zero",
  );
  ...
  assert(
    benchResult.processedIntervalCount > 9000,
    `Processed ${benchResult.processedIntervalCount} valid intervals`,
  );
  assert(
    benchResult.processingDurationMs < 5000,
    `Completed 10,000 intervals in ${benchResult.processingDurationMs}ms`,
  );
  assert(
    benchResult.throughputPerSec > 1000,
    `Throughput: ${benchResult.throughputPerSec} intervals/sec`,
  );
