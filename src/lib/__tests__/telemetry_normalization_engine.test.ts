...
async function runTelemetryEngineTests() {
  ...
  const eskomParser = ParserRegistry.selectParser(
    "eskom_amr_feb_2026.csv",
    "Date,Time,kW,kVAr,kVA,PF",
  );
  assert(
    eskomParser.parserName === "EskomAMRParser",
    `Selected EskomAMRParser for Eskom AMR format (actual: ${eskomParser.parserName})`,
  );

  const muniParser = ParserRegistry.selectParser(
    "city_power_municipal_telemetry.csv",
    "Timestamp,CumulativekWh,Status",
  );
  assert(
    muniParser.parserName === "MunicipalAMRParser",
    `Selected MunicipalAMRParser for municipal dial format (actual: ${muniParser.parserName})`,
  );

  const vendorParser = ParserRegistry.selectParser(
    "schneider_pulse_export.csv",
    "Timestamp,Pulses,Meter_ID",
  );
  assert(
    vendorParser.parserName === "VendorSpecificParser",
    `Selected VendorSpecificParser for vendor pulse format (actual: ${vendorParser.parserName})`,
  );

  const genericParser = ParserRegistry.selectParser("unknown_data.csv", "ColA,ColB,ColC");
  assert(
    genericParser.parserName === "GenericCSVParser",
    `Selected GenericCSVParser fallback for generic CSV (actual: ${genericParser.parserName})`,
  );
  ...
  assert(
    eskomRes.records[0].active_energy_kwh === 25000,
    "Active energy kWh matches 30-min calculation (25000 kWh)",
  );
  assert(eskomRes.records[0].quality_status === "measured", "Quality status is 'measured'");
  assert(
    eskomRes.records[0].raw_payload?.rawKw === "50000",
    "Original raw payload preserved untouched",
  );
  ...
  const muniRes = TelemetryNormalizationEngine.normalizeTelemetry(
    "municipal_dial.csv",
    cumulativeCsv,
  );
  assert(muniRes.intervalMinutes === 15, "Detected 15-minute interval duration");
  assert(
    muniRes.records[1].active_energy_kwh === 150,
    "Calculated delta between 999800 and 999950 (150 kWh)",
  );
  assert(muniRes.rolloverCount === 1, "Counter rollover detected successfully");
  assert(
    muniRes.records[2].quality_status === "rollover",
    "Rollover record explicitly flagged with quality_status = 'rollover'",
  );
  assert(
    muniRes.records[2].active_energy_kwh === 150,
    "Calculated modulo rollover delta (1000000 - 999950 + 100 = 150 kWh)",
  );
  ...
  assert(
    gapRes.gapEvents[0].missingIntervals === 2,
    "Identified 2 missing intervals in gap window",
  );
  assert(
    gapRes.gapEvents[0].estimationPermitted === true,
    "Calculated estimation permitted (gap <= 2h)",
  );
  assert(
    gapRes.records.length === 4,
    "Created 2 explicitly flagged estimated records + 2 measured records (total 4)",
  );

  const estimatedRecords = gapRes.records.filter((r) => r.quality_status === "estimated");
  assert(
    estimatedRecords.length === 2,
    "Distinguishable estimated records explicitly flagged ('quality_status = estimated')",
  );
  assert(
    estimatedRecords[0].active_power_kw === 42000,
    "Linear estimated active power (42000 kW interpolated between 40000 and 46000)",
  );
  ...
  assert(
    oooRes.records[0].local_timestamp.includes("13:00:00") ||
      oooRes.records[0].timestamp_utc.includes("11:00:00"),
  ...
  assert(
    oooRes.records.some((r) => r.quality_status === "duplicate"),
    "Duplicate record explicitly flagged ('quality_status = duplicate')",
  );
  ...
  assert(
    metrics.completenessPercent === 100,
    `Completeness % is 100% with gap estimation (actual: ${metrics.completenessPercent}%)`,
  );
  assert(
    metrics.validityPercent === 100,
    `Validity % is 100% (actual: ${metrics.validityPercent}%)`,
  );
  assert(
    metrics.estimatedPercent === 50,
    `Estimated % matches 2/4 records (actual: ${metrics.estimatedPercent}%)`,
  );
  assert(
    metrics.overallQualityScore > 90,
    `Overall quality score computed (> 90%) (actual: ${metrics.overallQualityScore}%)`,
  );
