...
import type { AuditEventRecord, AuditEventType, ReproducibleRunSnapshot } from '../../domain/audit/types';
...
async function runAuditTestSuite() {
  ...
  const sampleSnapshot: ReproducibleRunSnapshot = {
    ...
    tariff_snapshot: { tariff_code: 'ESKOM_MEGAFLEX_HV_2025_2026', peak_rate: 6.6692, std_rate: 2.1215 },
  ...
  assert(retrieved?.calculation_engine_version === '2.0.0', 'Retrieved exact calculation engine version (2.0.0)');
  ...
  assert(integrityResult.is_valid, 'Full cryptographic hash chain verified 100% valid & untampered');
  ...
  assert(!tamperedResult.is_valid, 'Successfully detected tampering in intermediate hash chain event');
  assert(tamperedResult.tampered_sequence_numbers.includes(loggedEvents[4].sequence_number), 'Accurately identified tampered sequence number');
  assert(tamperedResult.first_broken_sequence_number === loggedEvents[4].sequence_number, 'Identified exact first broken sequence number');
