...
describe("Adversarial Principal Audit Suite (First Principles)", () => {
  ...
  it("2. DEFECT PROBE: TOU classification boundary timestamp determinism in SAST (+02:00)", () => {
    ...
    const domainTariffDef = {
      header: {
        tariff_code: megVersion.family,
        version: megVersion.effectiveFrom,
        effective_date: megVersion.effectiveFrom,
      },
      components: [],
      public_holidays: [],
      tou_schedule: [
    ...
    console.log(
      "[PROBE 2] 06:00 SAST Interval Classification -> Legacy:",
      touLegacy,
      "| TouScheduleEngine:",
      touSchedule,
    );
  ...
  it("8. DEFECT PROBE: Missing interval time gaps in input series", () => {
    const rows: Measurement[] = [
      {
        ts: new Date("2026-02-16T17:30:00Z"),
        kW: 5000,
        kVAr: 1000,
        kVA: 5100,
        pf: 0.98,
      ...
      {
        ts: new Date("2026-02-17T00:00:00Z"),
        kW: 5000,
        kVAr: 1000,
        kVA: 5100,
        pf: 0.98,
  ...
  it("9. DEFECT PROBE: Duplicate timestamps in raw meter data", () => {
    const rows: Measurement[] = [
      {
        ts: new Date("2026-02-16T12:00:00Z"),
        kW: 5000,
        kVAr: 1000,
        kVA: 5100,
        pf: 0.98,
  ...
  it('10. DEFECT PROBE: Uploading custom PDF with "february" in filename', async () => {
    const customPdfContent = new TextEncoder().encode(
      "Custom Invoice for Customer ABC Corp, Total Due: R 500,000.00",
    );
    const customFile = new File([customPdfContent], "SiteB_February_2026_Invoice.pdf", {
  ...
  it("16. DEFECT PROBE: FinancialMath precision eliminates IEEE 754 float binary drift", () => {
    ...
    console.log(
      "[PROBE 16] IEEE 754 Float Sum:",
      floatSum,
      "| FinancialMath Decimal Sum:",
      decimalSum,
    );
