...
async function runStreamingPipelineTests() {
  ...
  assert(
    largeSummary.rowsSeen === 1000,
    `Rows seen matches 1000 input lines (actual: ${largeSummary.rowsSeen})`,
  );
  assert(
    largeSummary.rowsImported === 1000,
    `All 1000 valid synthetic rows imported (actual: ${largeSummary.rowsImported})`,
  );
  ...
  const dupSummary = await StreamingIngestionService.processStreamingIngestion({
    name: "large_meter_telemetry_1k.csv",
  ...
  assert(
    malformedSummary.status === "completed_with_warnings",
    "Job completed with warnings status",
  );
  assert(
    malformedSummary.errors.some((e) => e.errorCode === "MISMATCHED_COLUMN_COUNT"),
    "Mismatched column count error recorded",
  );

  // Test 4: Missing Required Columns in Header
  console.log("\n--- Test 4: Missing Required Header Columns ---");
  const missingColCsv = ["RandomCol1,RandomCol2,RandomCol3", "Val1,Val2,Val3"].join("\n");

  const missingColSummary = await StreamingIngestionService.processStreamingIngestion({
    name: "missing_columns.csv",
    size: missingColCsv.length,
    content: missingColCsv,
  });

  assert(
    missingColSummary.status === "failed",
    "Job failed when required timestamp/active power header columns are missing",
  );
  assert(
    missingColSummary.errors.some((e) => e.errorCode === "MISSING_REQUIRED_COLUMN"),
    "Missing required column error logged",
  );
  ...
  const extraColSummary = await StreamingIngestionService.processStreamingIngestion({
    name: "extra_columns.csv",
    size: extraColCsv.length,
    content: extraColCsv,
  });

  assert(
    extraColSummary.warnings.some((w) => w.includes("unexpected column")),
    "Transformation warning logged for unexpected extra columns",
  );
  ...
  const invalidTsSummary = await StreamingIngestionService.processStreamingIngestion({
    name: "invalid_ts.csv",
    size: invalidTsCsv.length,
    content: invalidTsCsv,
  });

  assert(invalidTsSummary.rowsRejected === 2, "Rejected 2 invalid timestamp rows");
  assert(
    invalidTsSummary.errors.filter((e) => e.errorCode === "INVALID_TIMESTAMP").length === 2,
    "Logged 2 INVALID_TIMESTAMP error records",
  );
  ...
  const invalidNumSummary = await StreamingIngestionService.processStreamingIngestion({
    name: "invalid_num.csv",
    size: invalidNumCsv.length,
    content: invalidNumCsv,
  });

  assert(invalidNumSummary.rowsRejected === 2, "Rejected non-numeric kW rows");
  assert(
    invalidNumSummary.errors.some((e) => e.errorCode === "INVALID_NUMERIC"),
    "INVALID_NUMERIC error code logged",
  );
  ...
  const negativeSummary = await StreamingIngestionService.processStreamingIngestion({
    name: "negative_values.csv",
    size: negativeCsv.length,
    content: negativeCsv,
  });

  assert(negativeSummary.rowsRejected === 1, "Rejected negative active power row (-50000 kW)");
  assert(
    negativeSummary.errors.some((e) => e.errorCode === "NEGATIVE_IMPOSSIBLE_VALUE"),
    "NEGATIVE_IMPOSSIBLE_VALUE critical error logged",
  );
  assert(
    negativeSummary.errors.some((e) => e.errorCode === "INVALID_POWER_FACTOR_BOUNDS"),
    "INVALID_POWER_FACTOR_BOUNDS warning logged for pf 2.5",
  );
