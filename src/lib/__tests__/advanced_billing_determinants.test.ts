...
function runAdvancedDeterminantsTests() {
  ...
  assert(highPfRes.pf_calculated!.gt(new Decimal('0.99')), `Calculated vector PF (${highPfRes.pf_calculated!.toString()}) > 0.99`);
  ...
  assert(lowPfRes.pf_calculated!.lt(new Decimal('0.96')), `Calculated vector PF (${lowPfRes.pf_calculated!.toString()}) < 0.96`);
  assert(lowPfRes.excess_kvarh!.gt(new Decimal(0)), `Excess kVARh derived (${lowPfRes.excess_kvarh!.toString()} kVARh)`);
  assert(lowPfRes.value.gt(new Decimal(0)), `Reactive penalty amount calculated (R ${lowPfRes.value.toFixed(2)})`);
  ...
  assert(boundaryPfRes.excess_kvarh!.lte(new Decimal('0.01')), `Boundary excess kVARh is zero (${boundaryPfRes.excess_kvarh!.toString()})`);
  ...
  assert(zeroEnergyRes.status === 'ZERO_ENERGY_NO_PENALTY', 'Zero energy returns status ZERO_ENERGY_NO_PENALTY');
  ...
  assert(zeroKvaRes.pf_calculated!.eq(new Decimal('0.00')), 'Zero active kWh evaluates PF to 0.00 without throwing zero division');
  ...
  assert(missingReactiveRes.status === 'MISSING_REACTIVE_DATA', 'Missing reactive data returns status MISSING_REACTIVE_DATA');
  assert(missingReactiveRes.value.eq(new Decimal(0)), 'Missing reactive data returns zero penalty without fabricating numbers');
  ...
  assert(missingDemandRes.status === 'MISSING_DEMAND_DATA', 'Missing demand data returns status MISSING_DEMAND_DATA');
  assert(missingDemandRes.value.eq(new Decimal(0)), 'Missing demand data returns zero billing demand without fabricating numbers');
  ...
  assert(ratchetRes.billing_demand_kva!.eq(new Decimal('3500.00')), `Billing demand ratcheted to 3,500 kVA (actual: ${ratchetRes.billing_demand_kva!.toString()})`);
  ...
  assert(fullSummary.active_energy_kwh.eq(new Decimal('1250000')), `Calculated active energy total (${fullSummary.active_energy_kwh.toString()} kWh)`);
  assert(fullSummary.utilised_capacity_percent.eq(new Decimal('97.00')), `Utilised capacity percentage (${fullSummary.utilised_capacity_percent.toString()}%)`);
  assert(fullSummary.audit_trace.length >= 2, `Generated ${fullSummary.audit_trace.length} audit steps`);
