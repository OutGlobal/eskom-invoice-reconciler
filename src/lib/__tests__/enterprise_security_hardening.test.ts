...
const superAdminCtx = createSecurityContext(
  "usr-001",
  "superadmin@eskomreconciler.co.za",
  "org-001",
  "SUPER_ADMIN",
);
const orgAdminCtx = createSecurityContext(
  "usr-002",
  "admin@acme.co.za",
  "org-001",
  "ORGANISATION_ADMIN",
);
const energyManagerCtx = createSecurityContext(
  "usr-003",
  "manager@acme.co.za",
  "org-001",
  "ENERGY_MANAGER",
...
assert(
  hasPermission(superAdminCtx, "PERM_MANAGE_ORGANISATION"),
  "Super Admin has PERM_MANAGE_ORGANISATION",
);
assert(hasPermission(orgAdminCtx, "PERM_MANAGE_USERS"), "Org Admin has PERM_MANAGE_USERS");
assert(
  !hasPermission(orgAdminCtx, "PERM_MANAGE_ORGANISATION"),
  "Org Admin denied PERM_MANAGE_ORGANISATION",
);
assert(
  hasPermission(energyManagerCtx, "PERM_UPLOAD_FILES"),
  "Energy Manager has PERM_UPLOAD_FILES",
);
...
assert(
  !hasPermission(readOnlyCtx, "PERM_RUN_RECONCILIATION"),
  "Read Only denied PERM_RUN_RECONCILIATION",
);
...
const tenantACtx = createSecurityContext(
  "usr-user1",
  "user1@orgA.co.za",
  "org-A-101",
  "ORGANISATION_ADMIN",
...
assert(
  crossTenantRes.reason?.includes("UNAUTHORIZED_TENANT_ACCESS"),
  "Reason contains UNAUTHORIZED_TENANT_ACCESS",
);
...
dangerousFormulas.forEach((formula) => {
  ...
  assert(
    ...
    `Formula '${formula}' sanitized with leading quote -> '${sanitized}'`,
  );
});
...
assert(
  !oversizedFileRes.valid && oversizedFileRes.errorCode === "FILE_TOO_LARGE",
  "Oversized file (>50MB) blocked",
);

const executableScriptRes = validateUploadedFile("malicious_script.sh", "text/x-shellscript", 1024);
assert(
  !executableScriptRes.valid && executableScriptRes.errorCode === "EXECUTABLE_FILE_BLOCKED",
  "Shell script upload BLOCKED",
);

const exeFileRes = validateUploadedFile("malware.exe", "application/octet-stream", 2048);
assert(
  !exeFileRes.valid && exeFileRes.errorCode === "EXECUTABLE_FILE_BLOCKED",
...
const logWithSecret =
  "Error connecting with Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.e30.SecretToken and password='supersecretpassword'";
...
assert(
  !redactedLog.includes("eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9"),
  "JWT token redacted from log",
);
