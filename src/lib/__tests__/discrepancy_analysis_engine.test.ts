...
async function runDiscrepancyTestSuite() {
  ...
  assert(res1.diagnoses.some((d) => d.reason_code === 'METER_CLOCK_DRIFT'), 'Detected METER_CLOCK_DRIFT root cause');
  assert(res1.diagnoses.some((d) => d.reason_code === 'INCORRECT_TIMEZONE'), 'Detected INCORRECT_TIMEZONE root cause');
  assert(res1.diagnoses.some((d) => d.reason_code === 'MISSING_INTERVALS'), 'Detected MISSING_INTERVALS root cause');
  assert(res1.diagnoses.some((d) => d.reason_code === 'DUPLICATE_INTERVALS'), 'Detected DUPLICATE_INTERVALS root cause');
  assert(res1.diagnoses.some((d) => d.reason_code === 'METER_RESET'), 'Detected METER_RESET root cause');
  assert(res1.diagnoses.some((d) => d.reason_code === 'DATA_QUALITY_ISSUE'), 'Detected DATA_QUALITY_ISSUE root cause');
      ...
      comparisons: [
        {
          ...
          unit: 'kWh',
          tolerance: { component_code: 'PEAK_KWH', component_name: 'Peak Energy', absolute_tolerance_zar: new Decimal('10'), percentage_tolerance: new Decimal('0.001'), unit: 'kWh' },
          status: 'MATERIAL_DISCREPANCY',
        ...
        },
        {
          ...
          unit: 'kWh',
          tolerance: { component_code: 'TOTAL_KWH', component_name: 'Total Energy', absolute_tolerance_zar: new Decimal('10'), percentage_tolerance: new Decimal('0.001'), unit: 'kWh' },
          status: 'MATCH',
          reason_code: 'MATCH',
        },
      ],
      discrepancies: [
        {
          ...
          unit: 'ZAR',
          tolerance: { component_code: 'ACTIVE_ENERGY_CHARGES', component_name: 'Active Energy', absolute_tolerance_zar: new Decimal('10'), percentage_tolerance: new Decimal('0.001'), unit: 'ZAR' },
          status: 'MATERIAL_DISCREPANCY',
        ...
        },
        {
          ...
          percentage_variance: new Decimal('0.05'),
          unit: 'ZAR',
          tolerance: { component_code: 'ALL_LINE_ITEMS', component_name: 'All Items', absolute_tolerance_zar: new Decimal('10'), percentage_tolerance: new Decimal('0.001'), unit: 'ZAR' },
          status: 'MATERIAL_DISCREPANCY',
  ...
  assert(res2.diagnoses.some((d) => d.reason_code === 'INCORRECT_TOU_SCHEDULE'), 'Diagnosed INCORRECT_TOU_SCHEDULE root cause');
  assert(res2.diagnoses.some((d) => d.reason_code === 'INCORRECT_SEASON'), 'Diagnosed INCORRECT_SEASON root cause');
  assert(res2.diagnoses.some((d) => d.reason_code === 'INCORRECT_TARIFF_VERSION'), 'Diagnosed INCORRECT_TARIFF_VERSION root cause');
  ...
  const utilCtx: DiagnosticInputContext = {
    ...
    reconciliationRun: {
      ...
      comparisons: [
        ...
        },
        {
          ...
          unit: 'ZAR',
          tolerance: { component_code: 'REACTIVE_PENALTY_CHARGES', component_name: 'Reactive Energy', absolute_tolerance_zar: new Decimal('10'), percentage_tolerance: new Decimal('0.001'), unit: 'ZAR' },
          status: 'MATERIAL_DISCREPANCY',
        ...
        },
        {
          ...
          unit: 'ZAR',
          tolerance: { component_code: 'NETWORK_CHARGES', component_name: 'Network Charges', absolute_tolerance_zar: new Decimal('10'), percentage_tolerance: new Decimal('0.001'), unit: 'ZAR' },
          status: 'MATERIAL_DISCREPANCY',
    ...
    extractedInvoice: {
      ...
      validation_summary: {
        ...
        discrepancies: [
          { rule_id: 'val-1', rule_name: 'Subtotal VAT', severity: 'critical', expected_value: 71739, actual_value: 78000, message: 'Subtotal + VAT sum mismatch' },
        ],
      },
    } as any,
  };
  ...
  assert(res3.diagnoses.some((d) => d.reason_code === 'INCORRECT_DEMAND_DETERMINANT'), 'Diagnosed INCORRECT_DEMAND_DETERMINANT');
  assert(res3.diagnoses.some((d) => d.reason_code === 'RATCHET_APPLIED_INCORRECTLY'), 'Diagnosed RATCHET_APPLIED_INCORRECTLY');
  assert(res3.diagnoses.some((d) => d.reason_code === 'REACTIVE_CALCULATION_MISMATCH'), 'Diagnosed REACTIVE_CALCULATION_MISMATCH');
  assert(res3.diagnoses.some((d) => d.reason_code === 'NETWORK_CHARGE_MISMATCH'), 'Diagnosed NETWORK_CHARGE_MISMATCH');
  assert(res3.diagnoses.some((d) => d.reason_code === 'VAT_CALCULATION_MISMATCH'), 'Diagnosed VAT_CALCULATION_MISMATCH');
  assert(res3.diagnoses.some((d) => d.reason_code === 'INVOICE_EXTRACTION_ERROR'), 'Diagnosed INVOICE_EXTRACTION_ERROR');
  assert(res3.diagnoses.some((d) => d.reason_code === 'METER_TO_INVOICE_MAPPING_ERROR'), 'Diagnosed METER_TO_INVOICE_MAPPING_ERROR');

  // --- Scenario 4: All 22 Root Causes Validation & Summary Output ---
  console.log('\n--- Test 4: Comprehensive Discrepancy Diagnostics Summary ---');
  assert(res3.total_diagnoses > 0, `Generated ${res3.total_diagnoses} diagnoses in comprehensive run`);
  assert(res3.total_disputed_financial_impact_zar.greaterThan(new Decimal('0')), 'Calculated total disputed financial impact in ZAR');
  assert(res3.high_confidence_count > 0, `Counted ${res3.high_confidence_count} high confidence deterministic diagnoses`);
