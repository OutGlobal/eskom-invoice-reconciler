...
async function runDatabaseSchemaTests() {
  ...
  assert(
    ...
    "Default organisation record present (Impala Platinum)",
  );
  ...
  assert(
    !ingestionJobsError,
    `Read ingestion_jobs domain table (error: ${ingestionJobsError?.message})`,
  );

  const { error: parserResultsError } = await supabase.from("parser_results").select("*").limit(1);
  assert(
    !parserResultsError,
    `Read parser_results domain table (error: ${parserResultsError?.message})`,
  );
  ...
  assert(
    !telemetryError,
    `Read telemetry_intervals domain table (error: ${telemetryError?.message})`,
  );
  ...
  assert(
    !componentError,
    `Read tariff_components domain table (error: ${componentError?.message})`,
  );
  ...
  const { data: reasonCodes, error: reasonError } = await supabase
    .from("discrepancy_reason_codes")
    .select("*");
  assert(
    !reasonError,
    `Read discrepancy_reason_codes domain table (error: ${reasonError?.message})`,
  );
  assert(
    Array.isArray(reasonCodes) && reasonCodes.length >= 4,
    "Taxonomy seed data present in discrepancy_reason_codes",
  );

  const { error: discEventError } = await supabase.from("discrepancy_events").select("*").limit(1);
  assert(
    !discEventError,
    `Read discrepancy_events domain table (error: ${discEventError?.message})`,
  );
  ...
  const { error: calcSnapError } = await supabase
    .from("calculation_snapshots")
    .select("*")
    .limit(1);
  assert(
    !calcSnapError,
    `Read calculation_snapshots domain table (error: ${calcSnapError?.message})`,
  );
  ...
  assert(
    !disputePackError,
    `Read dispute_packs domain table (error: ${disputePackError?.message})`,
  );
