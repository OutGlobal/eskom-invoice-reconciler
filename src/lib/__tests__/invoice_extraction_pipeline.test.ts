...
async function runInvoiceExtractionTests() {
  ...
  const pages = PageClassifier.classifyPages([
    ESKOM_MEGAFLEX_PDF_TEXT,
    "Meter dial reading schedule summary",
  ]);
  assert(pages.length === 2, "Classified 2 document pages");
  assert(
    pages[0].classification === "tax_invoice_header",
    "Page 1 classified as tax_invoice_header",
  );
  assert(
    pages[1].classification === "meter_reading_schedule",
    "Page 2 classified as meter_reading_schedule",
  );
  ...
  assert(
    eskomDoc.account_number.value === "ACC-78901234",
    `Extracted account number (${eskomDoc.account_number.value})`,
  );
  assert(
    eskomDoc.invoice_number.value === "INV-2026-03-9988",
    `Extracted invoice number (${eskomDoc.invoice_number.value})`,
  );
  assert(
    eskomDoc.tariff_name.value === "Eskom Megaflex",
    `Extracted tariff name (${eskomDoc.tariff_name.value})`,
  );
  ...
  assert(
    eskomDoc.standard_kwh.value === 600000,
    `Extracted standard kWh (${eskomDoc.standard_kwh.value})`,
  );
  assert(
    eskomDoc.off_peak_kwh.value === 400000,
    `Extracted off-peak kWh (${eskomDoc.off_peak_kwh.value})`,
  );
  assert(
    eskomDoc.maximum_demand.value === 4850,
    `Extracted maximum demand (${eskomDoc.maximum_demand.value} kVA)`,
  );
  assert(
    eskomDoc.notified_maximum_demand.value === 5000,
    `Extracted NMD (${eskomDoc.notified_maximum_demand.value} kVA)`,
  );
  assert(
    eskomDoc.total_invoice_amount.value === 920000,
    `Extracted total invoice amount (R ${eskomDoc.total_invoice_amount.value})`,
  );

  // Verify Mandatory Field Contract Attributes
  assert(
    eskomDoc.account_number.field_name === "account_number",
    "Field structure contains field_name",
  );
  assert(eskomDoc.account_number.source_page === 1, "Field structure contains source_page");
  assert(
    eskomDoc.account_number.source_text_reference.length > 0,
    "Field structure contains source_text_reference",
  );
  assert(
    eskomDoc.account_number.confidence_score >= 0.85,
    `High confidence score (${eskomDoc.account_number.confidence_score})`,
  );
  assert(
    eskomDoc.account_number.parser_version.includes("v1.0"),
    "Field structure contains parser_version",
  );
  ...
  assert(
    munDoc.account_number.value === "MUN-COJ-887711",
    `Extracted municipal account number (${munDoc.account_number.value})`,
  );
  assert(
    munDoc.total_invoice_amount.value === 3498875,
    `Extracted municipal total amount (R ${munDoc.total_invoice_amount.value})`,
  );
  assert(
    munDoc.line_items.length === 3,
    `Detected 3 tabular line items (actual: ${munDoc.line_items.length})`,
  );
  assert(
    munDoc.line_items[0].charge_label === "Peak Energy Charge",
    `Line item 1 label (${munDoc.line_items[0].charge_label})`,
  );
  assert(
    munDoc.line_items[0].invoiced_amount.value === 1027500,
    `Line item 1 invoiced amount (${munDoc.line_items[0].invoiced_amount.value})`,
  );
  ...
  assert(
    validSummary.financial_reconciled === true,
    "Subtotal + VAT financial sum reconciled cleanly",
  );
  ...
  assert(
    brokenSummary.discrepancies.length >= 2,
    `Logged ${brokenSummary.discrepancies.length} discrepancy events`,
  );
  ...
  assert(
    ocrDoc.account_number.confidence_score <= 0.8,
    `Scanned OCR confidence score adjusted (${ocrDoc.account_number.confidence_score})`,
  );
  assert(
    ocrDoc.metadata.needs_human_review === true,
    "Scanned low-confidence document flagged for human review",
  );
