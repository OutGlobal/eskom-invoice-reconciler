  ...
  public static async extractDocument(input: RawInputDocument): Promise<ExtractedInvoiceDocument> {
      ...
      for (let pageIdx = 0; pageIdx < input.pageTexts.length; pageIdx++) {
        ...
        for (const pattern of patterns) {
          ...
          if (match && match[1]) {
            ...
            if (
              pattern.source.includes("account|acc") ||
              pattern.source.includes("invoice\\s*no")
            ) {
    ...
    for (let l = 0; l < lines.length; l++) {
      ...
      const lineItemMatch = line.match(
        /^([A-Za-z0-9\s\/-]{5,40})\s+([0-9]+\.[0-9]+)\s+([0-9]+\.[0-9]+)\s+([0-9]+\.[0-9]+)$/,
      );
      if (lineItemMatch) {
    ...
    const needsHumanReview =
      overallConfidence < HIGH_CONFIDENCE_THRESHOLD || lowConfidenceFields.length > 0;
