...
export class PageClassifier {
  ...
  public static classifyPages(pageTexts: string[]): ClassifiedPage[] {
    return pageTexts.map((text, index) => {
      ...
      const hasTaxInvoiceHeader =
        /tax\s*invoice|account\s*number|vat\s*reg|invoice\s*date|amount\0? due/i.test(text);
      const hasLineItems =
        /charge\s*code|rate\s*\(|tariff\s*description|subtotal|total\s*charges/i.test(text);
      const hasMeterReading =
        /meter\s*(?:no|number)|dial\s*reading|kwh|kva|kvarh|peak\s*demand/i.test(text);
      const hasAnnexure =
        /payment\s*methods|terms\s*and\s*conditions|notice|important\s*information/i.test(text);
