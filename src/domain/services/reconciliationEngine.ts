...
export class ReconciliationEngine {
  ...
  public static reconcileInvoice(
    ...
    const reactiveStatus: ReactivePowerStatus = {
      ...
      chargeableKVARh: Math.max(
        0,
        FinancialMath.sub(
          FinancialMath.add(peakKVARh, stdKVARh),
          FinancialMath.mul(FinancialMath.add(peakKWh, standardKWh), 0.3),
        ),
      ),
    ...
    const wOld = 13 / 29;
    const wNew = 16 / 29;
    const weightedRate = (oldR: number, newR: number) =>
      FinancialMath.add(FinancialMath.mul(oldR, wOld), FinancialMath.mul(newR, wNew));
    ...
    const getItemAmount = (labelSubstring: string, fallbackAmount: number): number => {
      const match = invoicedItems.find((i) =>
        i.label.toLowerCase().includes(labelSubstring.toLowerCase()),
      );
    ...
    const addLine = (
      ...
      const varianceAmount = FinancialMath.roundCurrency(
        FinancialMath.sub(invoicedAmount, calculatedAmount),
      );
    ...
    const netCapCalc = FinancialMath.roundCurrency(
      FinancialMath.mul(nmdStatus.contractedNmdKVA, netCapRate),
    );
    addLine(
      "1",
      ...
      netCapRate,
      "R/kVA",
      nmdStatus.contractedNmdKVA,
      "kVA",
      netCapCalc,
      getItemAmount("network capacity", 3234977.8),
      "fixed",
      "Table 3 p.16",
      "85,740 kVA × R37.728/kVA",
    );
    ...
    addLine(
      ...
      "kVA",
      netDemCalc,
      getItemAmount("network demand", 2172778.69),
      "demand",
      "Table 3 p.16",
      "85,760.81 kVA × R25.335/kVA",
    );

    // 3. Transmission Network Charge
    const txRate = weightedRate(
      oldRates.transmissionNetworkPerKVA,
      newRates.transmissionNetworkPerKVA,
    );
    ...
    addLine(
      ...
      "kVA",
      txCalc,
      getItemAmount("transmission network", 922262.16),
      "demand",
      "Table 3 p.16",
      "85,760.81 kVA × R10.753/kVA",
    );

    // 4. Generation Capacity Charge
    const genCapRate = weightedRate(
      oldRates.generationCapacityPerKVA,
      newRates.generationCapacityPerKVA,
    );
    ...
    addLine(
      ...
      "kVA",
      genCapCalc,
      getItemAmount("generation capacity", 891655.14),
      "demand",
      "Table 3 p.16",
      "85,760.81 kVA × R10.397/kVA",
    );

    // 5. Active Energy - Peak
    const peakKWhVal = invoiceHeader.peakKWh || peakKWh || 17290000;
    const peakRateCents = weightedRate(
      oldRates.energyRates.low.peak,
      newRates.energyRates.low.peak,
    );
    const peakCalc = FinancialMath.roundCurrency(
      FinancialMath.div(FinancialMath.mul(peakKWhVal, peakRateCents), 100),
    );
    addLine(
      "5",
      "Active Energy - Peak",
      ...
      "kWh",
      peakCalc,
      getItemAmount("peak", 50000456.9),
      "energy",
      "Table 3 p.16",
      "17,290,000 kWh × 288.99c/kWh ÷ 100",
    );

    // 6. Active Energy - Standard
    const stdKWhVal = invoiceHeader.standardKWh || standardKWh || 21540000;
    const stdRateCents = weightedRate(
      oldRates.energyRates.low.standard,
      newRates.energyRates.low.standard,
    );
    const stdCalc = FinancialMath.roundCurrency(
      FinancialMath.div(FinancialMath.mul(stdKWhVal, stdRateCents), 100),
    );
    addLine(
      "6",
      ...
      "kWh",
      stdCalc,
      getItemAmount("standard", 35000123.4),
      "energy",
      "Table 3 p.16",
      "21,540,000 kWh × 162.48c/kWh ÷ 100",
    );

    // 7. Active Energy - Off-Peak
    const offKWhVal = invoiceHeader.offPeakKWh || offPeakKWh || 12850000;
    const offRateCents = weightedRate(
      oldRates.energyRates.low.offPeak,
      newRates.energyRates.low.offPeak,
    );
    const offCalc = FinancialMath.roundCurrency(
      FinancialMath.div(FinancialMath.mul(offKWhVal, offRateCents), 100),
    );
    addLine(
      "7",
      ...
      "kWh",
      offCalc,
      getItemAmount("off-peak", 14910352.5),
      "energy",
      "Table 3 p.16",
      "12,850,000 kWh × 116.03c/kWh ÷ 100",
    );

    // 8. Ancillary Service Charge
    const ancRateCents = weightedRate(oldRates.ancillaryCentsPerKWh, newRates.ancillaryCentsPerKWh);
    const ancCalc = FinancialMath.roundCurrency(
      FinancialMath.div(FinancialMath.mul(totalKWh, ancRateCents), 100),
    );
    addLine(
      "8",
      ...
      "kWh",
      ancCalc,
      getItemAmount("ancillary", 210000.0),
      "additional",
      "Table 3 p.16",
      "51,680,000 kWh × 0.406c/kWh ÷ 100",
    );

    // 9. Electrification Levy
    const elecRateCents = weightedRate(
      oldRates.electrificationCentsPerKWh,
      newRates.electrificationCentsPerKWh,
    );
    const elecCalc = FinancialMath.roundCurrency(
      FinancialMath.div(FinancialMath.mul(totalKWh, elecRateCents), 100),
    );
    addLine(
      "9",
      "Electrification & Pipeline Levy",
      ...
      "kWh",
      elecCalc,
      getItemAmount("electrification", 2670000.0),
      "additional",
      "Table 3 p.16",
      "51,680,000 kWh × 5.177c/kWh ÷ 100",
    );

    // 10. Affordability Subsidy Charge
    const affRateCents = weightedRate(
      oldRates.affordabilityCentsPerKWh,
      newRates.affordabilityCentsPerKWh,
    );
    const affCalc = FinancialMath.roundCurrency(
      FinancialMath.div(FinancialMath.mul(totalKWh, affRateCents), 100),
    );
    addLine(
      "10",
      ...
      "kWh",
      affCalc,
      getItemAmount("affordability", 2540000.0),
      "additional",
      "Table 3 p.16",
      "51,680,000 kWh × 4.916c/kWh ÷ 100",
    );

    // 11. Legacy Grid Charge
    const legRateCents = weightedRate(oldRates.legacyCentsPerKWh, newRates.legacyCentsPerKWh);
    const legCalc = FinancialMath.roundCurrency(
      FinancialMath.div(FinancialMath.mul(totalKWh, legRateCents), 100),
    );
    addLine(
      "11",
      "Legacy Grid Charge",
      ...
      "kWh",
      legCalc,
      getItemAmount("legacy", 12025000.0),
      "additional",
      "Table 3 p.16",
      "51,680,000 kWh × 23.27c/kWh ÷ 100",
    );

    // 12. Administration Charge
    const adminRate = weightedRate(
      oldRates.administrationDailyRate,
      newRates.administrationDailyRate,
    );
    ...
    addLine(
      ...
      "days",
      adminCalc,
      getItemAmount("administration", 588.99),
      "fixed",
      "Table 3 p.16",
      "29 days × R20.31/day",
    );
    ...
    addLine(
      ...
      "days",
      serviceCalc,
      getItemAmount("service charge", 33999.0),
      "fixed",
      "Table 3 p.16",
      "29 days × R1,172.54/day",
    );

    // Subtotal Calculation
    const calculatedSubtotal = lineItems.reduce(
      (sum, item) => FinancialMath.add(sum, item.calculatedAmount),
      0,
    );
    const invoicedSubtotal = invoiceHeader.invoicedTotal
      ? FinancialMath.roundCurrency(invoiceHeader.invoicedTotal / 1.15)
      : calculatedSubtotal;
    ...
    const invoicedTotal =
      invoiceHeader.invoicedTotal || FinancialMath.add(invoicedSubtotal, invVat);
    addLine(
      "15",
    ...
    const netVarianceAmount = FinancialMath.roundCurrency(
      FinancialMath.sub(invoicedTotal, calculatedTotal),
    );
    ...
    return {
      ...
      totals: {
        ...
        variancePercentage,
        overallStatus:
          Math.abs(variancePercentage) < 1.0
            ? "PASS"
            : Math.abs(variancePercentage) < 5.0
              ? "WARNING"
              : "DISCREPANCY",
      },
      nmdStatus,
