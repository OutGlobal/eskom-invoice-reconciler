...
export class ReconciliationEngine {
  ...
  public static reconcileInvoice(
    ...
    const determinants = DeterminantEngine.calculateDeterminants({
      ...
      notified_maximum_demand_kva: input.notified_maximum_demand_kva || new Decimal(inv.notified_maximum_demand.value || 0),
      reactive_energy_kvarh: input.reactive_energy_kvarh || new Decimal(inv.reactive_energy_kvarh.value || 0),
    ...
    // 1. Peak kWh
    compareComponent('PEAK_KWH', 'Peak Energy (kWh)', new Decimal(inv.peak_kwh.value || 0), determinants.peak_kwh, 'kWh', 'TOU_CLASSIFICATION');
    
    // 2. Standard kWh
    compareComponent('STANDARD_KWH', 'Standard Energy (kWh)', new Decimal(inv.standard_kwh.value || 0), determinants.standard_kwh, 'kWh', 'TOU_CLASSIFICATION');

    // 3. Off-Peak kWh
    compareComponent('OFF_PEAK_KWH', 'Off-Peak Energy (kWh)', new Decimal(inv.off_peak_kwh.value || 0), determinants.off_peak_kwh, 'kWh', 'TOU_CLASSIFICATION');

    // 4. Total kWh
    compareComponent('TOTAL_KWH', 'Total Energy (kWh)', new Decimal(inv.total_kwh.value || 0), determinants.active_energy_kwh, 'kWh', 'DATA_QUALITY');

    // 5. Demand kVA
    compareComponent('DEMAND_KVA', 'Maximum Demand (kVA)', new Decimal(inv.maximum_demand.value || 0), determinants.billing_demand_kva, 'kVA', 'DEMAND_VARIANCE');

    // 6. Reactive Energy kVARh
    compareComponent('REACTIVE_KVARH', 'Reactive Energy (kVARh)', new Decimal(inv.reactive_energy_kvarh.value || 0), determinants.reactive_energy_kvarh, 'kVARh', 'REACTIVE_ENERGY_VARIANCE');

    // 7. Network Charges
    const calcNetwork = getCalculatedChargeSum(['NETWORK_DEMAND', 'NETWORK_CAPACITY', 'TRANSMISSION_NETWORK']);
    compareComponent('NETWORK_CHARGES', 'Network Charges', new Decimal(inv.network_charges.value || 0), calcNetwork, 'ZAR', 'NETWORK_CHARGE_VARIANCE');

    // 8. Capacity Charges
    const calcCapacity = getCalculatedChargeSum(['GENERATION_CAPACITY']);
    compareComponent('CAPACITY_CHARGES', 'Capacity Charges', new Decimal(inv.capacity_charges.value || 0), calcCapacity, 'ZAR', 'CAPACITY_VARIANCE');

    // 9. Service Charges
    const calcService = getCalculatedChargeSum(['SERVICE_CHARGE', 'ADMINISTRATION_CHARGE']);
    compareComponent('SERVICE_CHARGES', 'Service Charges', new Decimal(inv.service_charges.value || 0), calcService, 'ZAR', 'ROUNDING_VARIANCE');

    // 10. Reliability Services
    const calcReliability = getCalculatedChargeSum(['ANCILLARY_SERVICE']);
    compareComponent('RELIABILITY_SERVICES', 'Reliability Services', new Decimal(inv.reliability_services.value || 0), calcReliability, 'ZAR', 'LEVY_VARIANCE');

    // 11. Levies
    const calcLevies = getCalculatedChargeSum(['ELECTRIFICATION_SUBSIDY', 'AFFORDABILITY_SUBSIDY']);
    compareComponent('LEVIES', 'Levies & Subsidies', new Decimal(inv.levies.value || 0), calcLevies, 'ZAR', 'LEVY_VARIANCE');

    // 12. VAT
    compareComponent('VAT_AMOUNT', 'VAT Amount (15%)', new Decimal(inv.vat_amount.value || 0), tariffCalc.vat_amount, 'ZAR', 'VAT_VARIANCE');

    // 13. Total Bill
    const billedTotal = new Decimal(inv.total_invoice_amount.value || 0);
    const expectedTotal = tariffCalc.total_inc_vat;
    compareComponent('TOTAL_BILL', 'Total Invoice Amount', billedTotal, expectedTotal, 'ZAR', 'MATERIAL_DISCREPANCY');
    ...
    const hasRoundingOnly = discrepancies.every((d) => d.status === 'MATCH' || d.status === 'ROUNDING_VARIANCE');
    ...
    if (expectedTotal.gt(0)) {
      varPct = totalVarZar.abs().div(expectedTotal).mul(100).toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
    }
