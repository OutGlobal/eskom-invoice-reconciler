...
import { classifyTou, type TouPeriod } from "@/lib/tariff";

export class LargeDatasetQueryEngine {
  private static datasetCache = new Map<string, TelemetryIntervalRecord[]>();
   ...
   */
  public static registerDataset(datasetId: string, records: TelemetryIntervalRecord[]): void {
   ...
   */
  public static getDataset(datasetId: string): TelemetryIntervalRecord[] | undefined {
   ...
   */
  public static clearDatasetCache(): void {
   ...
   */
  public static async queryPaginatedIntervals(
    filter: TelemetryQueryFilter,
    pagination?: PaginationOptions,
    sourceIntervals?: TelemetryIntervalRecord[],
    ...
    let candidateRecords: TelemetryIntervalRecord[] = [];
    ...
    const encodeCursor = (pageNumber: number) => {
   ...
   */
  public static async aggregateIntervalsForCharts(
    filter: TelemetryQueryFilter,
    cadence: AggregationCadence = "day",
    maxBuckets = 300,
    sourceIntervals?: TelemetryIntervalRecord[],
    context?: UserSecurityContext,
    options?: { includeDetailedBuckets?: boolean },
    ...
    const bucketsMap = new Map<
      string,
      {
        ...
        outageCount: number;
        touCounts: Record<TouPeriod, number>;
      }
    >();
    ...
    const aggregatedBuckets: AggregatedTimeSeriesBucket[] = [];
    const chartFormattedSeries: ChartPlottablePoint[] = [];
    ...
    const qualityScore = Number(
      Math.max(
        0,
        (1 - (totalEstimated + totalOutages) / Math.max(1, filtered.length)) * 100,
      ).toFixed(1),
    );
   ...
   */
  public static async streamBatchProcessor<T, R>(
    items: T[],
    processBatch: (batch: T[], batchIndex: number) => Promise<R[]> | R[],
    options?: BatchProcessingOptions,
  ): Promise<BatchProcessingResult & { results: R[] }> {
    const startTime = performance.now();
  ...
  private static applyFilters(
    ...
    const filterEnd = filter.endDate
      ? new Date(
          filter.endDate.includes("T") ? filter.endDate : filter.endDate + "T23:59:59.999Z",
        ).getTime()
      : undefined;
    ...
    return records.filter((rec) => {
      ...
      // Channel matching
      if (channelSet && rec.channel && !channelSet.has(rec.channel as any)) {
        return false;
      }
   ...
   */
  private static getBucketKey(dt: Date, cadence: AggregationCadence, tou?: TouPeriod): string {
   ...
   */
  private static coalesceBuckets(rawBuckets: any[], maxBuckets: number): any[] {
   ...
   */
  private static createEmptyChartResponse(
    meterId: string,
