...
import type {
  ...
  DetectedHeaders,
  DetectedSchemaType,
  IntervalIngestionOptions,
  IntervalProcessingSummary,
  MissingGapRecord,
  TelemetryGapEvent,
...
export interface IngestionEngineResult {
  ...
  >;
  summary: IntervalProcessingSummary;
  errors: string[];
  warnings: string[];
}

interface ParsedTimestampResult {
  dateObj: Date;
  localTs: string;
  isoUtc: string;
}
...
export class AmrIntervalIngestionEngine {
  ...
  public static processIntervalStream(
    filename: string,
    contentOrBuffer: string | ArrayBuffer | Uint8Array,
    options?: IntervalIngestionOptions,
    ...
    if (
      !detectedHeaders.timestampColumn &&
      !(detectedHeaders.dateColumn && detectedHeaders.timeColumn)
    ) {
      errors.push(
        "NO_TIMESTAMP_COLUMN_DETECTED: Could not identify a valid Date/Time column in file headers.",
      );
    ...
    const hasEnergyFields =
      Boolean(detectedHeaders.activePowerColumn) ||
      Boolean(detectedHeaders.activeEnergyColumn) ||
      Boolean(detectedHeaders.cumulativeRegisterColumn);

    if (!hasEnergyFields) {
      errors.push(
        "NO_ENERGY_COLUMNS_DETECTED: No active power (kW), energy (kWh), or register dial columns found in file headers.",
      );
      return this.createFailureResult(filename, parsedStructure, errors, warnings, detectedHeaders);
    }

    // Step 5: Identify Meter Identifier
    const detectedMeterId = this.identifyMeter(
      filename,
      fileStructure.preambleLines,
      detectedHeaders,
      dataRows,
      options?.meterId,
    );
    ...
    const detectedDurationMinutes = this.detectIntervalDuration(
      rawParsedRows.map((r) => r.dateObj),
    );
    ...
    const gapEvents: Array<
      TelemetryGapEvent & { startLocal?: string; endLocal?: string; missingCount?: number }
    > = [];
    const missingGapRecords: MissingGapRecord[] = [];
    const missingIntervalDetails: Array<{
    ...
    let suspectCount = 0;
    let rolloverCount = 0;
    const MAX_DIAL_CAP = 1_000_000; // 1M dial reset ceiling
    ...
    for (let i = 0; i < rawParsedRows.length; i++) {
      ...
      if (
        current.rawCumulative !== undefined &&
      ...
      let apparentPowerKva =
        current.rawKva !== undefined && !isNaN(current.rawKva) ? current.rawKva : 0;
      ...
      const record: CanonicalEnergyRecord &
        CanonicalTelemetryRecord & {
          ts: Date;
          kW: number;
          kVAr: number;
        ...
        meter_id:
          detectedHeaders.meterColumn && current.rawRow?.[detectedHeaders.meterColumn]
        ...
        interval_minutes: detectedDurationMinutes as 5 | 15 | 30 | 60,
          ...
          if (missingIntervals > 0) {
            ...
            if (options?.allowEstimation && estimationPermitted) {
              ...
              for (let step = 1; step <= missingIntervals; step++) {
                ...
                const interpKw = Number(
                  (activePowerKw + (nextKw - activePowerKw) * ratio).toFixed(2),
                );
    ...
    if (duplicateCount > 0) {
      warnings.push(
        `DUPLICATES_DETECTED: Found ${duplicateCount} duplicate timestamp records in stream.`,
      );
    }
    ...
    const avgPowerFactor =
      records.length > 0
        ? Number(
            (
              records.reduce((sum, r) => sum + (r.power_factor || 0.96), 0) / records.length
            ).toFixed(4),
          )
        : 0.96;
    ...
    if (records.length > 0) {
      ...
      const gapPenalty = Math.min(
        30,
        (totalMissingIntervals / (records.length + totalMissingIntervals)) * 100,
      );
      qualityScore = Math.max(
        0,
        Math.round(100 - (duplicatePenalty + suspectPenalty + gapPenalty)),
      );
    }
    ...
    const durationDays = Math.max(
      1,
      Math.ceil((new Date(endUtc).getTime() - new Date(startUtc).getTime()) / (86400 * 1000)),
    );

    const validCount = records.filter(
      (r) => r.quality_status === "measured" || r.quality_status === "validated",
    ).length;

    const summary: IntervalProcessingSummary = {
      meterId: detectedMeterId,
      fileStructure,
   ...
   */
  private static detectStructureAndParseRows(
    filename: string,
    contentOrBuffer: string | ArrayBuffer | Uint8Array,
  ): {
      ...
      try {
        ...
        for (const sName of wb.SheetNames) {
          ...
          const hasTelemetry = matrix.some((row) => {
            ...
            const hasTime =
              rowStr.includes("date") || rowStr.includes("time") || rowStr.includes("timestamp");
            const hasMetric =
              rowStr.includes("kw") ||
              rowStr.includes("kwh") ||
        ...
        for (let i = 0; i < Math.min(25, rawMatrix.length); i++) {
          ...
          const hasTime = rowStrings.some(
            (c) => c.includes("date") || c.includes("time") || c.includes("timestamp"),
          );
          const hasMetric = rowStrings.some(
            (c) =>
              c.includes("kw") ||
              c.includes("kwh") ||
              c.includes("kvar") ||
        ...
        for (let i = headerRowIndex + 1; i < rawMatrix.length; i++) {
          ...
          if (!rowVals || rowVals.length === 0 || !rowVals.some((c) => c !== "")) continue;
    ...
    const headerRow = this.splitCsvLine(headerLine, detectedDelimiter).map((h) =>
      h.trim().replace(/^["']|["']$/g, ""),
    );
    ...
    for (let i = headerRowIndex + 1; i < allLines.length; i++) {
      ...
      if (!line) continue;
      const parts = this.splitCsvLine(line, detectedDelimiter).map((p) =>
        p.trim().replace(/^["']|["']$/g, ""),
      );
      const rowObj: Record<string, any> = {};
      for (let c = 0; c < headerRow.length; c++) {
   ...
   */
  private static identifyColumns(headers: string[], headerRowIndex: number = 0): DetectedHeaders {
   ...
   */
  private static identifyMeter(
    filename: string,
    preambleLines: string[],
    headers: DetectedHeaders,
    rows: any[],
    configuredMeterId?: string,
  ): string {
    if (configuredMeterId) return configuredMeterId;
    ...
    for (const line of preambleLines) {
      if (line.includes(":") || line.includes("=")) {
        ...
        if (
          ...
          if (matchVal) return matchVal[1].trim();
        }
      }
    }
    ...
    if (fnMeterMatch) return fnMeterMatch[1];
  ...
  private static parseTimestamp(
    ...
    headers: DetectedHeaders,
    _defaultTimezone: string = "Africa/Johannesburg",
   ...
   */
  private static detectIntervalDuration(timestamps: Date[]): 5 | 15 | 30 | 60 {
    ...
    if (modeMinutes <= 8) return 5;
    if (modeMinutes <= 20) return 15;
    if (modeMinutes <= 45) return 30;
    return 60;
  }
   ...
   */
  private static isZipOrOfl(bytes: Uint8Array): boolean {
  ...
  private static createFailureResult(
    ...
    const emptyHeaders: DetectedHeaders = headers || {
      rawHeaders: structure.headerRow || [],
    };

    const emptyStructure: DetectedFileStructure = structure.fileStructure || {
      delimiter: ",",
      encoding: "UTF-8",
      headerRowIndex: 0,
      preambleRowCount: 0,
