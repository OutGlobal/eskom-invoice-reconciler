...
export class TelemetryStorageService {
    static BATCH_CHUNK_SIZE = 5000;
    static memoryIntervals = new Map();
    static memoryGaps = new Map();
    static memoryQuarantine = new Map();
    static recordIntervalsMemory(key, intervals) {
        const existing = this.memoryIntervals.get(key) || [];
        this.memoryIntervals.set(key, [...existing, ...intervals]);
    }
    static getIntervalsMemory(key) {
        return this.memoryIntervals.get(key) || [];
    }
    static recordGapsMemory(key, gaps) {
        const existing = this.memoryGaps.get(key) || [];
        this.memoryGaps.set(key, [...existing, ...gaps]);
    }
    static getGapsMemory(key) {
     ...
     */
    static async saveTelemetryBatch(params) {
        ...
        try {
            ...
            for (let i = 0; i < intervals.length; i += this.BATCH_CHUNK_SIZE) {
                ...
                const payload = chunk.map((item) => ({
                    ...
                    unit: item.unit,
                    source_file_id: item.source_file_id !== "src-file-local" && item.source_file_id.length > 10
                        ? item.source_file_id
                        : null,
                    ingestion_batch_id: item.ingestion_batch_id && item.ingestion_batch_id.length > 10
            ...
            if (quarantineRecords.length > 0) {
                const qPayload = quarantineRecords.map((q) => ({
                    ingestion_batch_id: q.ingestion_batch_id && q.ingestion_batch_id.length > 10
     ...
     */
    static async saveEstimation(estimation, estimatedInterval) {
        try {
            const payload = {
     ...
     */
    static computeSummary(intervals, quarantined, gaps, estimationsCount = 0) {
        const countsByState = {
            ACTUAL: 0,
            ESTIMATED: 0,
            INTERPOLATED: 0,
     ...
     */
    static async fetchIntervals(limit = 100, meterId) {
        try {
            let query = supabase
        ...
        if (meterId) {
            return (this.memoryIntervals.get(meterId) || []).slice(0, limit);
        }
     ...
     */
    static async saveIntervals(intervals) {
        if (intervals.length > 0) {
