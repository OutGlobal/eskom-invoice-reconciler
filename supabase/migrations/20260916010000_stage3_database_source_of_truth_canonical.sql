...
-- ==============================================================================
-- 15. SYNC INITIAL STORED INVOICES INTO INVOICE_RECORDS (IDEMPOTENT)
-- Ensures existing stored invoice records from early migrations populate canonical table
-- ==============================================================================
...
SELECT
    ...
    i.account_number,
    c.organisation_id,
    COALESCE(i.billing_period, 'Standard Period'),
    COALESCE(i.billing_start, '2026-02-01'::date),
    COALESCE(i.billing_end, '2026-02-28'::date),
    COALESCE(i.total_kwh, 0),
    COALESCE(i.peak_kwh, 0),
    COALESCE(i.standard_kwh, 0),
    COALESCE(i.off_peak_kwh, 0),
    COALESCE(i.max_demand_kva, 0),
    COALESCE(i.invoiced_total, 0),
    COALESCE(i.reconciled_total, 0),
    COALESCE(i.variance_amount, 0),
    CASE 
        WHEN LOWER(COALESCE(i.status, '')) = 'reconciled' THEN 'reconciled'
        WHEN LOWER(COALESCE(i.status, '')) = 'processed' THEN 'validated'
        ELSE 'draft'
    END,
    COALESCE(i.created_at, now())
FROM public.invoices i
LEFT JOIN public.customers c ON i.account_number = c.account_number
WHERE i.invoice_number IS NOT NULL AND i.invoiced_total > 0
ON CONFLICT (invoice_number) DO UPDATE SET
    organisation_id = COALESCE(EXCLUDED.organisation_id, invoice_records.organisation_id),
